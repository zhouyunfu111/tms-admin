...
var wx = {
	request(obj) {
		...
		return new Promise(async (resolve,reject) => {
			try {
				const json = await vm.$jsonp(obj.url, obj.data)
				if (json.status == 0) {
					resolve(json)
					obj.success(json)
				} else {
...
var Utils = {
	...
	handleData(param, data, feature) {
      ...
      var calculateDistanceResult = data.result.rows;
			var distance = [];
			for (var i = 0; i < calculateDistanceResult.length; i++) {
        for(var j = 0; j < calculateDistanceResult[i].elements.length; j++) {
          distance.push(calculateDistanceResult[i].elements[j].distance);
        }
			}
			param.success(data, {
...
class QQMapWX {
	...
	calculateDistance(options) {
		...
		if (requestParam.mode == 'straight') {
			...
			var locationsuccess = async function(result) {
				requestParam.from = result.latitude + ',' + result.longitude;
				if (options.sig) {
					requestParam.sig = Utils.getSig(requestParam, options.sig, 'calculateDistance');
				}
        try {
          return await wx.request(Utils.buildWxRequestConfig(options, {
            url: URL_DISTANCE,
            data: requestParam
          }, 'calculateDistance'));
        } catch(e) {
          ...
          return Promise.reject(e)
			...
			return Utils.locationProcess(options, locationsuccess);
		}
	};
